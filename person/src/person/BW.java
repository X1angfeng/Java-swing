...
public class BW extends JPanel implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3340810508772245365L;
	JLabel jl1;
	JTable jt1;
	...
	String userName="sa";
	String userPwd="123";
	String[] title = {"时间","地点","事件"};
	Object[][]  A={};
	int count;	
	public BW(String userId) {
		...
		jb1 = new JButton("备忘录被👽抓走了");
		jb2 = new JButton("备忘录管理");
		...
		try 
		{
			Connection con=DriverManager.getConnection(dbURL,userName,userPwd); 
		   	...
		   	String r1="select mtime,mlocation,mevent from memo where ID='"+userId+"'";
		    ...
		    while(rs.next())
		    {
		    	A[count][0]=rs.getString("mtime");
		    	A[count][1]=rs.getString("mlocation");
		    	A[count][2]=rs.getString("mevent");
	...
	public void actionPerformed(ActionEvent e) {
			...
			try 
		{
			Connection con=DriverManager.getConnection(dbURL,userName,userPwd);		   
			Statement s=con.createStatement( ); 
			String r1="select mtime,mlocation,mevent from memo  where ID='"+login.userId+"'";
		   ResultSet rs=s.executeQuery(r1);
			   	int count = 0;	
			   	while(rs.next())
			   	{
