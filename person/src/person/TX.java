...
public class TX extends JPanel implements ActionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = 7441777517271109275L;
	JLabel jl1;
	JTable jt1;
	...
	String userName="sa";
	String userPwd="123";
	String[] title = {"电话","姓名","城市","工作地点","备注"};
	Object[][]  A={};
	int count;	
	public TX(String userId) {
		...
		jb1 = new JButton("通讯录👽抓走了");
		jb2 = new JButton("通讯录管理");
		...
		try 
		{
			Connection con=DriverManager.getConnection(dbURL,userName,userPwd); 
		   	...
		   	String r1="select telephone,cname,city,place,remarks from communication where ID='"+userId+"'";
		    ...
		    while(rs.next())
		    {
		    	A[count][0]=rs.getString("telephone");
		    	A[count][1]=rs.getString("cname");
		    	A[count][2]=rs.getString("city");
		    	A[count][3]=rs.getString("place");
		    	A[count][4]=rs.getString("remarks");
	...
	public void actionPerformed(ActionEvent e) {
			...
			try 
		{
			Connection con=DriverManager.getConnection(dbURL,userName,userPwd);		   
			Statement s=con.createStatement( ); 
			String r1="select telephone,cname,city,place,remarks from communication  where ID='"+login.userId+"'";
		   ResultSet rs=s.executeQuery(r1);
			   	int count = 0;	
			   	while(rs.next())
			   	{
