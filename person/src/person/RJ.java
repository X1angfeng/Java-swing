...
public class RJ extends JPanel implements ActionListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -8305776933275779442L;
	JLabel jl1;
	JTable jt1;
	...
	String userName="sa";
	String userPwd="123";
	String[] title = {"时间","天气","事件"};
	Object[][]  A={};
	int count;	
	public RJ(String userId) {
		...
		jb1 = new JButton("日记被👽抓走了");
		jb2 = new JButton("日记管理");
		...
		try 
		{
			Connection con=DriverManager.getConnection(dbURL,userName,userPwd); 
		   	...
		   	String r1="select dtime,weather,devent from diary where ID='"+userId+"'";
		    ...
		    while(rs.next())
		    {
		    	A[count][0]=rs.getString("dtime");
		    	A[count][1]=rs.getString("weather");
		    	A[count][2]=rs.getString("devent");
	...
	public void actionPerformed(ActionEvent e) {
		   ...
		   if(e.getSource() == jb1)		//页面刷新
			{
			try 
		{
			Connection con=DriverManager.getConnection(dbURL,userName,userPwd);		   
			Statement s=con.createStatement( ); 
			String r1="select dtime,weather,devent from diary  where ID='"+login.userId+"'";
		   ResultSet rs=s.executeQuery(r1);
			   	int count = 0;	
			   	while(rs.next())
			   	{
